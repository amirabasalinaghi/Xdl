...
## Features
- Monitor a single X account feed for new reposts.
...
## Troubleshooting X token/account access
- `X_BEARER_TOKEN` does **not** need to be generated by the same X user as `X_USER_ID`.
- However, your X app/project must still be allowed to read the target account feed through the v2 API.
- If you get `401`/`403` from X:
  ...
  - Confirm your app has the required read scopes/permissions (`tweet.read`, `users.read`) and project access level needed for feed endpoints.
- The relay monitors a single account feed using `/users/{id}/tweets` only, which is compatible with standard app-only bearer polling.
